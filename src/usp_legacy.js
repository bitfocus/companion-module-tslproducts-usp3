...
module.exports = {
	...
	autoConfigureLegacyPanel: async function () {
		...
		try {
			...
			if (companionIP == '') {
				self.log(
					'error',
					'Could not find a network interface on the same subnet as the panel - unable to auto-configure.'
				)
				self.updateStatus(InstanceStatus.ConnectionFailure, 'See log for details.')
				return
			}
		} catch (error) {
		...
		try {
			...
			portfinder
				.getPortPromise({ port: 161, stopport: 200 })
				.then((port) => {
		...
		try {
			if (self.config.port_usp != '' && self.config.port_usp != undefined) {
				...
				try {
					...
					self.USP_LEGACY_SNMP_AGENT = snmp.createAgent(
						{ port: self.config.port_usp_legacy, accessControlModelType: snmp.AccessControlModelType.Simple },
						function (error, data) {
							if (error) {
								console.error(error)
	...
	updateLegacyKeys: function (key) {
		...
		try {
			...
			if (changed) {
				if (key.number <= 16) {
					//really only care about the first 16 keys
					key.changed = true
		...
		try {
			if (self.config.host_usp !== '' && self.config.host_usp !== undefined) {
				...
				let req = client.post('http://' + self.config.host_usp + '/tally.htm', args, function (data, response) {
					setTimeout(
						self.updateCurrentlyUpdatingVariable.bind(self),
						self.config.advanced_config_usp_legacy_panel_delay_time,
						false
					)
				})
