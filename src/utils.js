			...
			for (let index = 0; index < str_split.length; index++) {
				...
				if (command == 'BEGIN') {
					...
					self.sendCompanionSatelliteCommand(
						`ADD-DEVICE DEVICEID=${self.DEVICE_ID} PRODUCT_NAME="${productName}" BITMAPS=false COLORS=true TEXT=true`
					)
					continue
				}
				...
				if (command == 'KEY-STATE') {
					...
					if (self.DATA.brightness <= 50) {
						...
						let dimColor = self.COLORS.find((color) => {
							return color.label == closestColor.label + '_DIM'
						})
						if (dimColor) {
					...
					if (keyData.fontSize == undefined) {
						//if the font size is not already set, determine a size
					...
					if (self.config.model == 'usp3') {
						if (keyData.number <= 16) {
							//no need to send anything higher than 16
